...
### Added
- **빌더 branch/expand 오퍼레이터 노출 + operator DAG 관측성 + 컬럼 영향도 토글 (2026-06-19, 자율런)**: ① 오케스트레이션 스텝에 `branch` 필드(JSON `{when,to}[]`) — 자기 outcome predicate로 하류 선택, 나머지 skip(전파). `validateTasksDAG.branch_bad_target`(to가 직접 하류 아니면 경고). Airflow BranchPythonOperator를 UI에서. ② `expand` 필드(JSON `{key:list}`) — 리스트 원소마다 fan-out(리전/파티션별), `{{ map.<key> }}` 노출, cross product. 트리거룰·branch·expand 트리오 UI 완비. ③ **operator DAG 관측성 3종**: per-step node_runs(post_run 훅, 코어 변경 0) + **스텝별 행수**(RunResult.task_records → node_run R/W, 이전엔 0) + **branch-skip vs upstream-failure 구분**(result_json.task_state stash → "skipped"/"upstream failed" 라벨, enum/마이그레이션 0) + **expand fan-out 인스턴스 가시화**(RunResult.mapped_instances → "⑃ N instances · M failed" + 인스턴스별 성패/map_values/행수). ④ 자산 상세 컬럼 리니지 **Upstream/Impact 방향 토글** — 영향도 분석(`direction=downstream`: 소비 하류 추적, "바꾸면 뭐가 깨지나"). 백엔드 일반화 BFS + 케이스-인센서티브 fallback. 라이브: dim_customer 하류 5자산·12엣지, expand 데모 fanout 3인스턴스(us/eu/apac).

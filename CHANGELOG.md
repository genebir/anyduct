...
### Added
- **빌더 branch(조건 분기) 오퍼레이터 노출 + 컬럼 영향도 토글 (2026-06-19, 자율런)**: ① 오케스트레이션 스텝에 `branch` 필드(JSON `{when,to}[]`) — 이 스텝의 자기 outcome(records_read/written/success) predicate로 어느 하류 스텝이 실행될지 선택, 나머지 skip(전파). serialize/deserialize round-trip + `validateTasksDAG.branch_bad_target`(to가 실제 직접 하류가 아니면 실행 전 경고). Airflow BranchPythonOperator를 UI에서 구성(코어는 이미 ADR-0028 지원). ② 자산 상세 컬럼 리니지에 **Upstream/Impact 방향 토글** — 영향도 분석(`direction=downstream`: 이 컬럼을 소비하는 하류 컬럼 추적, "바꾸면 뭐가 깨지나"). 백엔드 `column_lineage_graph(direction=)` 일반화 BFS + 케이스-인센서티브 자산/컬럼 fallback. 라이브: dim_customer 하류 5자산·12엣지 2홉 전파. ③ operator DAG run에 **per-step node_runs**(post_run 훅으로 RunResult 포착 → task_states를 node_runs로, 코어 변경 0) — 성공/실패/branch-skip(cancelled) 가시화.

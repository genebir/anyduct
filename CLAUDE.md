...
**Step 8.5c 완료 → 8.5d Pipelines 차례** (2026-05-18 기준). Steps 1–4 + Step 5.1 + Step 7.0~7.4 + Step 8.1 + Step 8.2a/b(로컬 + OIDC JWT) + Step 8.3(RBAC) + Step 8.4(Audit log infra) + Step 8.5a(Workspaces CRUD) + Step 8.5b(Membership management) + **Step 8.5c(Connections CRUD + Secret backend 완전 통합 + `POST /test` — 6 엔드포인트, `{"$secret": "key"}` sentinel + secrets 맵으로 평문 0 보장, ConnectionTester가 sqlite 등 실 connector를 ConnectorRegistry로 호출)** 완료. **344 코어 단위 + 65 etlx-server 단위 + 3 skip + 119 코어 통합(testcontainers PG/MinIO/Kafka/MySQL/Vault/LocalStack) + 131 서버 통합 = 659 테스트** all green, **24 ADR**. `mypy strict` 코어 39 + 서버 50 src files OK, `lint-imports` 2 contracts KEPT. **OOP 정규화 일관 적용**: services(`JwtService`/`PasswordService`/`OidcService`/`OidcStateSigner`/`AuditService`/`SecretWalker`/`ConnectionTester`) + repository(`UserRepository`/`MembershipRepository`/`WorkspaceRepository`/`ConnectionRepository`/`AuditLogRepository`) + 도메인 라우터(`auth.py`/`oidc.py`/`workspaces.py`/`memberships.py`/`connections.py`/`audit.py`) + `Depends`-기반 DI(`get_current_workspace`/`require_workspace_role`/`get_audit_service`/`get_secret_backend_dep`) + 모듈 레벨 `_require_*` Depends 싱글톤(ruff B008 회피) + 도메인 예외(`WorkspaceSlugTakenError`/`MembershipExistsError`/`LastOwnerError`/`ConnectionNameTakenError`/`SecretMarkerError`/`SecretBackendReadOnlyError`)로 라우터에서 깨끗한 4xx/5xx 매핑.
...
- 🔄 Step 8 (API Server / FastAPI):
  ...
  - 8.5 a~e 분리 (도메인이 넓어 1 PR = 1 slice 유지)
    ...
    - 8.5c ✅ Connections CRUD + test — 6 엔드포인트(`/workspaces/{ws}/connections` + `/{id}/test`) + `etlx_server/connections/` 패키지(`ConnectionRepository`/`SecretWalker`/`ConnectionTester`) + secret marker `{"$secret": "key"}` + secret backend `app.state` lifecycle + `ConnectorRegistry`로 sqlite 등 실 connector 호출 + 평문 0 보장
    - 8.5d~e 미착수 — 다음은 Pipelines CRUD + `GET /{pid}/versions` (Step 7.3 yaml_sync의 PipelineVersion idempotency 재사용)
  - 8.6~8.7 미착수

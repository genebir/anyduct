...
> - examples/elt_pushdown.yaml(+자격 lint 가드). 검증: 코어 unit 1270 / 서버 it 520+ / 통합 226 / helm lint / Storybook 빌드 전부 green(매 슬라이스), web tsc clean, live 스택+kind 실주행. **다음 후보**: 10.8 a11y 게이트(axe test-runner — 의존 추가라 사용자 정렬) / 분포-기반 자동 분할 / DW Arrow fast-path(live 게이트) / v1.0 릴리스(user-gated).
>

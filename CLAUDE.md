...
- 📋 Step 9 (Execution Engine 통합) 미착수 — worker가 runs 큐 claim(SKIP LOCKED) → 코어 `run_pipeline_yaml` 호출 + heartbeat per ~30s + zombie 회수. Step 8.6에서 enqueue까지 끝났으니 9는 큐 consumer + status writer만 추가.

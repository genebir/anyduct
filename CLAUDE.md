...
- 🔄 Step 8 (API Server / FastAPI):
  ...
  - 8.7 미착수 — 다음은 시나리오 테스트(가입→워크스페이스→연결→파이프라인→실행→결과 end-to-end) 또는 **Step 9 (Execution Engine)** — 워커가 runs 큐 claim → 코어 `run_pipeline_yaml` 호출 + heartbeat + zombie 회수.

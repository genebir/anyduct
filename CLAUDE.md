...
**Step 8.5 (CRUD 도메인) 완전 종료 → 8.6 Action 엔드포인트 차례** (2026-05-18 기준). Steps 1–4 + Step 5.1 + Step 7.0~7.4 + Step 8.1 + Step 8.2a/b + Step 8.3 + Step 8.4 + **Step 8.5 전체(workspaces/memberships/connections/pipelines/schedules CRUD + runs read-only)** 완료. **344 코어 단위 + 65 etlx-server 단위 + 3 skip + 119 코어 통합(testcontainers PG/MinIO/Kafka/MySQL/Vault/LocalStack) + 171 서버 통합 = 699 테스트** all green, **24 ADR**. `mypy strict` 코어 39 + 서버 55 src files OK, `lint-imports` 2 contracts KEPT. **OOP 정규화 일관 적용**: services(`JwtService`/`PasswordService`/`OidcService`/`OidcStateSigner`/`AuditService`/`SecretWalker`/`ConnectionTester`) + repository(User/Workspace/Membership/Connection/Pipeline/Schedule/Run/AuditLog) + 도메인 라우터(`auth.py`/`oidc.py`/`workspaces.py`/`memberships.py`/`connections.py`/`pipelines.py`/`schedules.py`/`runs.py`/`audit.py`) + `Depends`-기반 DI + 모듈 레벨 `_require_*` Depends 싱글톤(ruff B008 회피) + 도메인 예외(`WorkspaceSlugTakenError`/`MembershipExistsError`/`LastOwnerError`/`ConnectionNameTakenError`/`SecretMarkerError`/`SecretBackendReadOnlyError`/`PipelineNameTakenError`/`InvalidCronError`)로 라우터에서 깨끗한 4xx/5xx 매핑.
...
- 🔄 Step 8 (API Server / FastAPI):
  ...
  - 8.6~8.7 미착수 — 다음은 Action 엔드포인트(`POST /pipelines/{id}/dry-run`/`trigger`, `POST /runs/{id}/retry`, `GET /runs/{id}/logs` streaming). Step 9 worker queue와 짝지어 — 8.6은 큐 enqueue만, 실제 execute는 Step 9.

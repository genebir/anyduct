...
**Step 8.5d 완료 → 8.5e Schedules+Runs 차례** (2026-05-18 기준). Steps 1–4 + Step 5.1 + Step 7.0~7.4 + Step 8.1 + Step 8.2a/b + Step 8.3 + Step 8.4 + Step 8.5a/b/c + **Step 8.5d(Pipelines CRUD + immutable version history — 6 엔드포인트, `PipelineRepository.ensure_version`로 config_json diff시에만 새 version 생성, audit.after_json에 `version_created` 플래그, core `PipelineConfig.model_validate`로 구조 검증, 서버가 `config["name"]` 주입)** 완료. **344 코어 단위 + 65 etlx-server 단위 + 3 skip + 119 코어 통합(testcontainers PG/MinIO/Kafka/MySQL/Vault/LocalStack) + 145 서버 통합 = 673 테스트** all green, **24 ADR**. `mypy strict` 코어 39 + 서버 53 src files OK, `lint-imports` 2 contracts KEPT. **OOP 정규화 일관 적용**: services(`JwtService`/`PasswordService`/`OidcService`/`OidcStateSigner`/`AuditService`/`SecretWalker`/`ConnectionTester`) + repository(`UserRepository`/`MembershipRepository`/`WorkspaceRepository`/`ConnectionRepository`/`PipelineRepository`/`AuditLogRepository`) + 도메인 라우터(`auth.py`/`oidc.py`/`workspaces.py`/`memberships.py`/`connections.py`/`pipelines.py`/`audit.py`) + `Depends`-기반 DI + 모듈 레벨 `_require_*` Depends 싱글톤(ruff B008 회피) + 도메인 예외(`WorkspaceSlugTakenError`/`MembershipExistsError`/`LastOwnerError`/`ConnectionNameTakenError`/`SecretMarkerError`/`SecretBackendReadOnlyError`/`PipelineNameTakenError`)로 라우터에서 깨끗한 4xx/5xx 매핑.
...
- 🔄 Step 8 (API Server / FastAPI):
  ...
  - 8.5 a~e 분리 (도메인이 넓어 1 PR = 1 slice 유지)
    ...
    - 8.5e 미착수 — 다음은 Schedules CRUD + 활성화 토글 + Runs 읽기 전용 (실행 트리거링은 Step 9)
  - 8.6~8.7 미착수

...
**Consequences**:
- ✅ 자동정렬이 노드 크기·접점·선 경로·배치·**여백**을 한 번에 최적화. 수동 보정(AKZ/ALA 핸들)은 그대로 유효(레이아웃이 라우팅 필드 소유 — 재정렬 시 초기화 후 채널 재배정).
...
- ⚠️ TB의 잔여 면적 증가는 정렬-가시성의 의도된 트레이드(충돌-인지 압축이 관통을 만들 이동을 거부).

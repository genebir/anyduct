...
- **Phase H — 노드 레벨 오케스트레이션(병렬)**:
  ...
  - [ ] **H2 스케줄러 노드 enqueue + 워커 노드 claim** — run 생성 시 graph를 node_runs로 펼침, 워커가 노드 단위 claim·실행(단일 노드 materialize + staging 핸드오프 결정)·하류 release. retry/heartbeat/reaper 재사용.

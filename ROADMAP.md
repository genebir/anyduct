...
- **Phase K — 부가(#6)**:
  - [x] **빌더 drawer 너비 조절(사용자 요청 — "Transform 영역 너비 조절 가능하도록")** ✅ (2026-05-26) — PropertiesPanel(drawer) 좌측 엣지에 col-resize 핸들 추가, pointer-events로 드래그(터치/마우스 공통), `setPointerCapture`로 드래그 중 캔버스 벗어나도 추적, **380~880px clamp**, 기본 520px, **localStorage `etlx.builder.drawerWidth`로 영속**(다음 세션·다른 페이지에서도 사용자 폭 유지). `_readStoredWidth()` private 모드/quota/NaN 모두 안전 fallback. 핸들은 hover 시 `bg-accent/40` highlight + `aria-label` i18n("드래그해서 속성 패널 너비 조절"). 드래그 중 `document.body.cursor`/`userSelect`로 글로벌 cursor + 선택 방지. 웹 tsc green.

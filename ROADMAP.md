...
- **Phase H — 노드 레벨 오케스트레이션(병렬)**:
  ...
  - [ ] **H2b 워커 노드 실행 결선** — run을 node_runs로 펼침 + run-소유 워커가 ready 노드를 in-memory 핸드오프로 동시 실행·하류 release·run 상태 집계. **scope 결정: v1은 in-process 단일 워커(cross-worker 분산 + persistent staging은 후속, H1 claim_ready가 seam).** retry/heartbeat 재사용.

...
**Steps 1–4 + Step 5.1(MySQL/SQLite) + Step 7.0~7.4 + Step 8 전체 + Step 9.3a(worker batch lifecycle) 완료. 코어 344 unit + 서버 65 unit + 3 skip + 코어 119 it + 서버 204 it = 735 테스트, 24 ADR.**
...
- [ ] **9.3b (heartbeat + zombie reaper) 미착수** — 현재 heartbeat는 claim 시점에만 stamp. 장시간 실행 중 stuck worker 검출하려면 worker가 N초마다 heartbeat 갱신 + 별도 `ZombieReaper`가 heartbeat 만료된 running row를 pending으로 회수.
- [ ] **9.3c (log/metric 수집) 미착수** — 코어 structlog → run_logs, 코어 metrics ABC → run_metrics. 현재 worker는 status + counter만 기록.

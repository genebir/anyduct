...
**Steps 1–4 + Step 5.1(MySQL/SQLite) + Step 5.3(MongoDB) + Step 5.7(HTTP) + Step 6.1 (Cursor abstraction + 4 connector read_since + Pipeline.run cursor backfill + cursor contract) + Step 6.2 부분(OTel 어댑터 + Pipeline span emit) + Step 7.0~7.4 + Step 8 전체 + Step 9.2(scheduler) + Step 9.3a + Step 9.3b(heartbeat + zombie reaper) 완료. 코어 435 unit + 서버 65 unit + 3 skip + 코어 165 it + 서버 223 it = 891 테스트, 24 ADR.**

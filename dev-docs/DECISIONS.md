...
## ADR-0092: ERD 자동정렬 — 렌더러-정확 기하 공유 + 접점 정렬/채널 최적화

**Date**: 2026-06-10
**Status**: Accepted
**Context**: 사용자 요구 *"자동정렬 시 노드 크기·접점 위치·연결선 위치·노드 간 위치를 함께 고려한 최적(최대 가시성) 배치"*. 기존 구조의 근본 문제: 레이아웃(dagre, 노드 중심 기준)과 선 라우팅(렌더 시점 floating anchor + 형제 분배 + 직선스냅 + 장애물 회피)이 서로를 모름 → 배치가 끝나도 접점이 어긋나 14px 스냅을 살짝 벗어나는 S-꺾임 대량 발생, 같은 갭의 세로 구간 겹침, 노드 높이 추정치(56+25n)와 실제 렌더 높이 불일치로 예측 자체가 부정확.

**Decision**:
1. **공유 기하 모듈** `lib/erd-edge-geometry.ts` — borderIntersection/rectSide/distributeAnchor/straightSnap/bestCenter/pathCrossings를 순수 함수로 추출, 렌더러(crowsfoot-edge)와 레이아웃(erd-layout)이 동일 코드 사용 → 레이아웃의 선 예측 = 실제 렌더.
2. **렌더 높이 결정화** — 테이블 노드 행/헤더에 lineHeight 고정(`round(16·scale)px`) → auto 높이가 닫힌 공식 `contentHeight(n,s) = 15 + lh + n(lh+9)`. 레이아웃은 rendered px(fontScale 반영)로 계산.
3. **레이아웃 파이프라인** (`autoLayout` 재작성, TB는 rect 전치로 동일 코드):
   - dagre pass 1 → 실제 앵커 면별 카운트 측정 → 허브 면 길이 보장(`(k+1)·EDGE_GAP`) + 갭 통과 엣지 수 기반 adaptive ranksep → dagre pass 2.
   - **정렬 스위프**: 교차축으로 노드를 움직여 연결 앵커쌍을 일치(Gauss-Seidel ≤18회, 수렴 감지). 노드별 시프트는 평균이 아닌 **후보 시프트 스코어링**(가장 많은 엣지를 직선화하는 델타에 커밋 — degree-2 체인이 양쪽 다 꺾이는 타협 방지). 랭크 내 순서·간격은 가중 PAVA(등방 회귀)로 보존.
   - **직선 레스큐**: 허브 팬 리프가 달성 불가능한 앵커에 끌려가며 달성 가능한 1:1 정렬을 밀어내는 문제 → 그리디 힐클라임(영향 엣지 증분 평가, 전역 직선 수가 늘 때만 수락; 교차점형 앵커의 언더슈팅은 내부 반복으로 수렴).
   - **채널 배정**: 남은 꺾인 엣지의 세로 구간 위치를 노드 관통 0 → 기존 채널과 ≥16px 분리 → 중앙 근접 순으로 선택, 렌더러 자체 선택(bestCenter)보다 나을 때만 `relation.centerRatio`로 영속(이후 노드 드래그에도 비율로 적응).
4. 진단 `analyzeRouting` export — 렌더러-정확 예측으로 직선 수/관통/세그먼트 겹침/노드 겹침 채점(드라이버·향후 테스트용).

**검증** (실 .damx 4종, 구 vs 신 동일 채점기): 직선 TMS 64→81(LR)/83(TB), CTC 24→33, 안전지원 41→46, 탭 합계 안전지원 49→69(+41%) · 노드 관통 −14~38%(안전지원 264→164) · 세로 구간 겹침 3~7→0 · 노드 겹침 0 유지 · 300테이블 ≤160ms · 멱등성 OK · fontScale 0.85/1.25/1.5 OK. web tsc clean + dev 200.

**Follow-up (같은 날, 여백 비대 수정)**: 사용자 피드백 *"여백이 너무 많아 ERD가 쓸데없이 커짐"*. 원인 2건 교정:
1. **갭별 랭크 간격(gapRetune)** — dagre의 전역 ranksep(붐비는 갭 하나가 모든 갭을 최대 400px로 인플레)을 폐기하고, 레이아웃 후 각 갭을 "그 갭에 실제로 세로 구간을 두는 **꺾인** 엣지 수"(스킵 엣지는 1/span 분담)만큼만 재배분(min 110/max 320). 직선 엣지는 갭 폭이 필요 없음.
2. **블록 압축(blockCompact)** — 정렬 스위프가 파트너를 쫓아 흩어 놓은 수직 구멍을, 직선 엣지로 연결된 노드 묶음(강체 블록, 평행이동은 내부 직선성 보존)을 x-겹침 선행 블록까지 위로 끌어모아 제거. **충돌-인지 수락**: 이동 영향권 엣지(블록 1-hop)의 박스 관통을 증분 채점해 관통이 늘면 1→½→¼ 시프트 폴백, 그래도 늘면 롤백 — 압축이 가시성을 되팔지 않음.
   + 레스큐가 "박스를 관통하는 직선"을 승리로 치지 않도록 클린 검사 추가, analyzeRouting도 직선 엣지의 박스 관통 포함(정직한 채점).
   결과(구 대비): LR 면적 −6~33%(TMS 368→347Mpx, 표준화 6→4) + 직선 +39~71%(TMS 64→89, CTC 24→41) + 관통 −4~45%(안전지원 258→143). TB 면적은 구(93~209Mpx, 관통 다수) 대비 +9~51% 남지만 1차 구현(187~227→)에서 대폭 회수, 관통은 절반.

**Follow-up 2 (같은 날, CTC 여백 재신고)**: ASCII 점유 맵 진단으로 밀도 4.3%의 진범 2건 확정·해결:
1. **skyline 클러스터 패킹** — shelf-pack은 한 클러스터가 지배적이면(CTC 102-노드, 높이 2.2만px) 행 높이가 그만큼 커져 나머지 19개 클러스터가 첫 행 상단 일렬 + 우하단 ~85% 공백. skyline(가장 낮은 곳 채움) + 후보 폭 4종(최광 클러스터/최고 클러스터 옆 채움/√면적×aspect) 중 면적·세장비 최적 선택으로 교체.
2. **serpentine wrap(리본 클러스터 감기)** — ERD는 FK 깊이가 얕아(3~4랭크) 대형 클러스터가 1:10+ 세로 리본이 됨(fit-view에서 지렁이). 신문 칼럼처럼 교차축을 밴드로 잘라 옆으로 배열: 노드는 **중심 기준**으로 밴드 배정(슬라이스 없음, 밴드는 x-분리라 통로 불요 — 허브-팬처럼 y가 빈틈없이 덮인 클러스터도 감김), 컷은 걸치는 엣지 최소 위치, 밴드 수×가중 후보 시뮬레이션에서 (면적 × 세장비 패널티 0.3·log₂ × 걸침 0.006/edge) 최소를 선택("안 감기" 베이스라인 포함). 밴드별 x 재정규화(부분 랭크 밴드가 전체 폭을 복제하지 않게) + 감은 후 blockCompact/rescue 재실행.
   결과(구 대비, 동일 채점기): **면적 −40~64%**(CTC LR 310→112Mpx, TMS LR 372→148, 안전지원 LR 182→83) · 밀도 2~3배(4.3→12~17.5%) · 세장비 정상화(0.1~0.2→0.9~2.2) · 그러면서 직선 +29~58%/관통 −23~77%(안전지원 LR 261→61) 유지. 멱등성/fontScale/오버랩 0/≤430ms.

**Follow-up 3 (2026-06-10, Phase ALC — "임포트한 CTC 느낌으로")**: 사용자가 DA# 원본 배치를 기준으로 제시. 정량화하니 사람 배치는 **레이어 정렬과 정반대 트레이드**: 밀도 44.3%/정사각/직선 1개/관통 791(선이 테이블 밑으로) — 선을 포기하고 **근접성(locality)** 으로 읽는 모자이크. 이를 재현하는 **`mosaicLayout` (컴팩트 모드)** 신설 + 기본 모드화(TB/LR은 select에 유지):
- 연결도 순서(컴포넌트 큰 순 → 허브부터 BFS, 이웃 degree 순)로 테이블을 나열하고, **이웃-친화 skyline**(이미 배치된 FK-이웃과의 평균 거리 + 높이 비용 최소 지점)에 채움. 정사각-약간-가로 목표 폭, 간격은 DA# 밀도(~44%)에 튜닝(GX 100/GY 110). 라우팅은 채널 패스만(랭크가 없으므로 정렬 패스 비적용).
- 실 4종 결과: DA# 원본과 밀도 동급(43.8~45.6% vs 44.3~65.5%)이면서 **캔버스는 더 작고**(CTC 64→30Mpx, TMS 97→59Mpx), 관통 1/3(CTC 791→247), 직선은 오히려 多(1→15), 겹침 15~95→0, 연관 테이블 거리 중앙값 1/3(3014→1061). ≤52ms, 멱등.
- ⚠️ 소형 다이어그램(표준화 16테이블)은 레이어 모드가 직선 우세(8/14 vs 2/14) — 모드 선택으로 해소.

**Consequences**:
- ✅ 자동정렬이 노드 크기·접점·선 경로·배치·**여백·캔버스 모양**을 한 번에 최적화. 기본 모드는 DA#-느낌 컴팩트 모자이크, 선-우선 레이어(TB/LR)는 선택 가능. 수동 보정(AKZ/ALA 핸들)은 그대로 유효(레이아웃이 라우팅 필드 소유 — 재정렬 시 초기화 후 채널 재배정).
- ⚠️ 허브 fan-in은 물리적으로 전부 직선 불가(파트너 높이 합 > 허브 변 길이) — 이런 엣지는 분리된 채널로 정리되는 것이 설계 의도.
- ⚠️ wrap 컷을 걸치는 엣지는 밴드 사이 골목(WRAP_GAP)으로 길게 도는 의도된 트레이드 — 지렁이 리본보다 낫다는 판단(필요시 wrap 비용 가중치로 조절).
- ⚠️ 노드 CSS(행 패딩/보더) 변경 시 `contentHeight` 공식과 동기 필수(양쪽 주석 명시).
- 스킵-레벨 엣지의 수평 런이 중간 랭크를 지나는 관통은 단일-꺾임 모델의 한계로 잔존(다중 꺾임 라우팅은 후속 후보).
...
- **P2c(2026-06-10 구현)**: same-connection 푸시다운 — 소스=싱크 인스턴스면 `INSERT INTO <table> <select>` 한 문장(무이동·원자적). `supports_sql_pushdown` opt-in(9 dialect, cassandra 제외), plain-식별자 가드, append+no-pre_sql 한정(ADR-0035 원자성 약속 보존). 부적격 → Arrow → Record 순 폴백 체인.
...
- **P2e(2026-06-12 구현) — graph trivial 체인 fast-path 라우팅**: 웹 빌더는 모든 파이프라인을 graph shape으로 저장하므로 P2b/P2c가 linear 한정이면 빌더 사용자는 영원히 Record 경로였음(침묵 성능 갭). `_try_graph_fast_paths`: 그래프가 `source → sink`(2노드) 또는 `source → sql(pushdown) → sink`(3노드, ADR-0094) 무필터 단일 경로 체인이면 **shadow Task로 기존 linear 헬퍼(`_try_sql_pushdown`/`_try_arrow_fast_path`)를 그대로 재사용** — 2노드 same-connection은 자동 푸시다운(P2c 동형), cross-connection 양단 Arrow-capable이면 bulk COPY(P2b 동형). 같은-DB 판정은 커넥션 이름(`SinkSpec.connection_name`) — 빌더가 sink에 두 번째 인스턴스를 mint하므로 인스턴스 동일성은 불가. 비-trivial(fan-in/when/다중 sink)은 materialize 엔진 그대로. PG 통합 graph→COPY 실검증.
...
- **그래프 trivial 체인도 대상 (같은 날 후속)**: 웹 빌더는 모든 파이프라인을 graph shape으로 저장(serializeGraph) — linear 한정이면 빌더 사용자는 영원히 비대상이 된다. `_try_graph_pushdown`: 그래프가 정확히 `source → sql(pushdown) → sink` 3노드 체인(edge when 없음, 단일 append sink)이고 **양 끝이 같은 커넥션 이름**이면 동일 합성. 그래프 빌더는 sink에 두 번째 인스턴스를 mint(데드락 가드)하므로 인스턴스 동일성 비교는 불가 — `SinkSpec.connection_name`(원본 커넥션 이름) 신설로 이름 비교(같은 이름=같은 DB, 푸시다운은 한 문장이라 데드락 우려 없음). `GraphNode.transform_spec`(raw dump) 신설. lint graph 분기도 "항상 무효" → 자격 검사로 교체. 비-trivial 그래프(fan-in/when/다중 sink)는 materialize 엔진 그대로. 서버 node-level 실행(opt-in node_runs)은 노드 단위 dispatch라 푸시다운 비적용(기본 경로만).
...
## ADR-0096: Helm 차트 + prod 이미지 커넥터 extras — k8s 배포 1급화

**Date**: 2026-06-12
**Status**: Accepted (구현 + kind 실배포 검증 완료)
...
**Consequences**:
- ✅ Step 11의 Helm 항목 닫힘 + compose prod 사용자도 connectors extras 수혜(동일 Dockerfile). helm lint clean, kind e2e green.
- ⚠️ 내장 Postgres는 단일 replica/PVC — 프로덕션 비권장(차트 values에 명시). Ingress/TLS는 기존 정책대로 범위 밖.

...
> `services/anyduct-web` (Step 10)의 디자인 마스터 문서. 토큰·컴포넌트·인터랙션의 단일 진실(SSOT). 변경 시 ADR-0018을 갱신.
>
...
| `--text-muted` | `#8B96BB` | 139 150 187 | placeholder, 비활성 *(2026-06-12 a11y: 구 #6E7AA0은 overlay 위 3.5:1 — AA 미달이라 상향)* |
| `--text-disabled` | `#4A557A` | 74 85 122 | disabled |
| **`--accent`** | **`#FF3D8B`** | 255 61 139 | **주요 CTA, 활성 인디케이터, 핵심 강조** *(채워진 accent 위 텍스트는 `--text-on-accent`; 어두운 배경 위 accent "텍스트"는 4.2:1이라 활성 텍스트엔 `--accent-hover` 사용)* |
| `--text-on-accent` | `#FFFFFF` | 255 255 255 | accent/error 채움 위 텍스트. **오너 결정(2026-06-12): 흰색 유지** — 핑크 위 3.3:1로 AA 미달이나 네이비 대안을 검토 후 미관상 기각. axe 게이트는 `.text-on-accent` 요소만 명시 제외(그 외 전부 AA 게이트 유지) |
...
| `--text-muted` | `#64748B` *(2026-06-12 a11y: 구 #94A3B8은 흰 배경 2.6:1)* |
...
폰트 파일은 `services/anyduct-web/public/fonts/`에 self-host (`PretendardVariable.woff2`, woff2-variations). CDN 의존 금지 — 프로덕션 컨테이너가 런타임 외부 의존 없이 동작해야 함.
...
브랜드 로고는 SVG로 별도 보관 (`services/anyduct-web/public/logos/`).
...
```ts
// services/anyduct-web/tailwind.config.ts
...
```ts
// services/anyduct-web/components/pipeline-builder/theme.ts
...
```ts
// services/anyduct-web/lib/motion.ts
...
2. **새 컴포넌트는 먼저 Figma에 → 토큰 검증 → Storybook → 본 코드.** Storybook은 `services/anyduct-web/.storybook/`.

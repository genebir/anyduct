...
# Table-reference extraction lives in ``core`` (ADR-0017 / ADR-0022 단방향
# 의존): the auto-create-table path in ``core.pipeline`` needs it, and core
# may not import this runtime module. We re-export so existing importers of
# ``runtime.sql_lineage.extract_referenced_tables`` (runtime.lineage, the
# service worker) are unaffected, and reuse ``_table_fq_name`` below.
from etl_plugins.core.sql_introspect import _table_fq_name, extract_referenced_tables
...
# ``extract_referenced_tables`` now lives in
# :mod:`etl_plugins.core.sql_introspect` (imported at the top of this

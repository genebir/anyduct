"""``anyduct-server`` admin CLI.

Operates on the metadata DB directly. The core ``anyduct`` CLI in
:mod:`etl_plugins.cli` stays DB-agnostic (ADR-0017 unidirectional dependency).

Subcommands:

* ``anyduct-server import-yaml <yaml_dir> --workspace <slug>``
* ``anyduct-server export-yaml --workspace <slug> --to <dir>``
* ``anyduct-server worker run`` — long-running worker process
...
from anyduct_server.connections import (
...
from anyduct_server.db.models.workspace import User
from anyduct_server.db.session import make_engine, make_session_factory
from anyduct_server.io.yaml_sync import export_workspace, import_yaml_dir
from anyduct_server.scheduler import Scheduler
from anyduct_server.sensors import SensorScheduler
from anyduct_server.settings import get_settings
from anyduct_server.worker import RunWorker, StreamWorker, ZombieReaper
from anyduct_server.worker.recorder import log_processor
...
app = typer.Typer(help="anyduct-server administration CLI (metadata DB sync, ops).")
...
def gen_jwt_keys_cmd(
    ...
    Local-dev convenience: run once, then ``anyduct server run`` (via the
    `anyduct` wrapper) exports the PEMs as ``AUTH_JWT_PRIVATE_KEY_PEM`` /
        ...
        try:
            ...
            try:
                ...
                typer.echo(
                    ...
                    "configure ANYDUCT_SECRET_BACKEND to a writable backend.",
                    err=True,
                )
                raise typer.Exit(1) from e
...
def sensor_scheduler_run_cmd(
    ...
    **Multi-replica safe** (K2 pattern): the due-sensor query holds
    ...
    #   * anyduct_server.sensors.builtins — service-side builtins that need
    #     metadata-DB access (asset_freshness)
    import anyduct_server.sensors.builtins  # noqa: F401 — register side-effects

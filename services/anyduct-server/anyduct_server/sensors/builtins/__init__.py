...
Adding a new service-side sensor:
    1. Drop a module here. Use ``@register_sensor("name")`` on the
       ...
       from :data:`anyduct_server.sensors.context.sensor_session_factory`).
    2. Import the module from this ``__init__`` so the side-effect runs.
    3. Add its type label to ``services/anyduct-web/lib/sensors.ts``
...
from anyduct_server.sensors.builtins import (  # noqa: F401
    asset_freshness,
    dataset_row_count,
    file_landed,
    lineage_arrival,

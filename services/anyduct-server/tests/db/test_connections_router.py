...
from anyduct_server.db.models import AuditLog, Connection, Membership, User, Workspace
...
async def test_post_creates_connection_with_secret(session: AsyncSession) -> None:
    ...
    assert placeholder.startswith("${SECRET:anyduct/")
    assert "s3cret!" not in resp.text
    expected_path = f"anyduct/{ws.id}/{body['id']}/db_password"

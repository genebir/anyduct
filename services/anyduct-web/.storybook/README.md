# anyduct-web Storybook (Step 10.8)
...
```bash
pnpm --filter @anyduct/web storybook        # dev on :6006
pnpm --filter @anyduct/web build-storybook  # static build to storybook-static/
```
...
```bash
pnpm --filter @anyduct/web build-storybook
pnpm dlx http-server services/anyduct-web/storybook-static -p 6006 --silent &
pnpm --filter @anyduct/web test-storybook --url http://127.0.0.1:6006
```

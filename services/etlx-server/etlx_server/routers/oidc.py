...
* ``GET /callback?provider=<name>&code=<code>&state=<state>`` — IdP returns
  ...
  the user, then mint an ``etlx`` access + refresh token pair.
...
from etlx_server.auth.current_user import get_jwt_service, get_oidc_service
...
from etlx_server.auth.user_repository import OidcEmailCollisionError, UserRepository
...
async def callback(
    ...
    """Exchange the authorization code and mint an ``etlx`` token pair."""
    _require_enabled(settings)
    try:

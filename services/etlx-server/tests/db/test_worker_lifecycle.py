...
async def test_executor_runs_spark_engine(session: AsyncSession, tmp_path: Path) -> None:
    """engine='spark' run goes through the Spark backend end-to-end (ADR-0031/0032).

    Gated on pyspark + a JVM (the worker image bundles these in deployment);
    skipped otherwise.
    """
    import os
    import shutil
    ...
    if not (os.environ.get("JAVA_HOME") or shutil.which("java")):
        pytest.skip("no JVM for Spark engine test")

    from pyspark.sql import SparkSession

    spark = (
        SparkSession.builder.master("local[1]")
        .appName("etlx-worker-test")
        .config("spark.ui.enabled", "false")
        .config("spark.sql.shuffle.partitions", "1")
        .getOrCreate()
    )
    src_dir, out_dir = str(tmp_path / "in"), str(tmp_path / "out")
    spark.createDataFrame(
        [{"id": 1, "amount": 5}, {"id": 2, "amount": 0}, {"id": 3, "amount": 9}]
    ).write.mode("overwrite").parquet(src_dir)

    ws = await _seed_workspace(session, slug="we-spark")
    await _seed_connection(
        session,
        workspace_id=ws.id,
        name="src",
        type="file",
        config={"path": src_dir, "format": "parquet"},
    )
    await _seed_connection(
        session,
        workspace_id=ws.id,
        name="dst",
        type="file",
        config={"path": out_dir, "format": "parquet"},
    )
    cfg = {
        "name": "spk",
        "engine": "spark",
    ...
    p, pv = await _seed_pipeline(session, workspace_id=ws.id, name="spk", config=cfg)
    run = await _seed_pending_run(
        session, workspace_id=ws.id, pipeline_id=p.id, pipeline_version_id=pv.id
    )
    claimed = await claim_pending_run(session, worker_id="worker-A")
    ...
    assert refreshed.status == RunStatus.SUCCEEDED, (refreshed.error_class, refreshed.error_message)
    assert refreshed.records_read == 3
    assert refreshed.records_written == 2
    # The backend created + stopped its own session (getOrCreate singleton), so
    # re-acquire one to read the output.
    spark2 = (
        SparkSession.builder.master("local[1]").config("spark.ui.enabled", "false").getOrCreate()
    )
    ids = sorted(r["id"] for r in spark2.read.parquet(out_dir).collect())
    assert ids == [1, 3]

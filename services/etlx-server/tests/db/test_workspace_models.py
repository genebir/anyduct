...
from etlx_server.db.models import Membership, PersonalAccessToken, User, Workspace
...
async def test_pat_prefix_unique(session: AsyncSession) -> None:
    ...
    await session.flush()
    session.add(
        PersonalAccessToken(user_id=u.id, name="ci", prefix="etlx_pat_abcd1234", token_hash="h1")
    )
    session.add(
        PersonalAccessToken(
            user_id=u.id, name="laptop", prefix="etlx_pat_abcd1234", token_hash="h2"
        )
    )
    with pytest.raises(IntegrityError):
        await session.flush()
